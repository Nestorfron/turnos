...
const DependenciaPanel = () => {
  ...
  // refs para controlar fetch solo una vez por id
  const turnosFetchedId = useRef(null);
  const funcionariosFetchedId = useRef(null);

  useEffect(() => {
    if (!dependencia && id) {
      console.log(`Fetch dependencia con id: ${id}`);
      fetchData(`dependencias/${id}`, (data) => {
        console.log("Datos dependencia recibidos:", data);
        setDependencia(data);
      });
    }
  }, [dependencia, id]);
  ...
  useEffect(() => {
    ...
    if (turnosFetchedId.current !== dependencia.id) {
      turnosFetchedId.current = dependencia.id;
      console.log(`Fetch turnos para dependencia id: ${dependencia.id}`);
      fetchData(`turnos?dependencia_id=${dependencia.id}`, (data) => {
        console.log("Datos turnos recibidos:", data);
        setTurnos(data);
      });
    }
  }, [dependencia?.id]);
  ...
  useEffect(() => {
    ...
    if (funcionariosFetchedId.current !== dependencia.id) {
      funcionariosFetchedId.current = dependencia.id;
      console.log("Fetch usuarios para filtrar funcionarios");
      fetchData("usuarios", (usuarios) => {
        console.log("Datos usuarios recibidos:", usuarios);
        const filtrados = usuarios.filter((u) => u.dependencia_id === dependencia.id);
        console.log("Funcionarios filtrados:", filtrados);
...
export default DependenciaPanel;
